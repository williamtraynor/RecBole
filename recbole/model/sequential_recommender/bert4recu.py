...
class BERT4RecU(SequentialRecommender):
    def __init__(self, config, dataset):
        super(BERT4RecU, self).__init__(config, dataset)
        ...
        # HERE IS WHERE MULTI-MODAL INFO COULD BE IMPLEMENTED AS STARTING POINT OF ITEM EMBEDDINGS
        self.user_embedding = nn.Embedding(
            self.n_users + 1, self.hidden_size, padding_idx=0
        )  # mask token add 1
    ...
    def forward(self, item_seq, user_id):
        # make user embedding correct shape
        #user_id = user_id[:,None,:].unsqueeze(0).expand_as(item_seq)
        ...
        user_id = user_id.unsqueeze(1).expand_as(item_seq)
        user_emb = self.user_embedding(user_id)
        item_emb = self.item_embedding(item_seq)

        """self.logger.info(f'''
                
                User Embedding Shape: {user_emb.shape}
                Item Embedding Shape: {item_emb.shape}
                pos_id: {position_ids.shape}
                Pos Embedding Shape: {position_embedding.shape}
                ''')"""

        input_emb = item_emb + position_embedding + user_emb
    ...
    def calculate_loss(self, interaction):
        ...
        Inter: {'hi'}
        USER ID: {user_id}
        USEr Embedding Shape: {self.user_embedding(user_id)[:,None,:].shape}
        Item Embedding Shape: {self.item_embedding(masked_item_seq).shape}
        Pos Embedding Shape: {self.position_embedding(position_ids).shape}
        ''')"""

        seq_output = self.forward(masked_item_seq, user_id)
        ...
        if self.loss_type == "BPR":
            ...
            pos_items_emb = self.item_embedding(pos_items) + self.user_embedding(user_id)  # [B mask_len H]
            neg_items_emb = self.item_embedding(neg_items) + self.user_embedding(user_id) # [B mask_len H]
    ...
    def predict(self, interaction):
        ...
        scores = torch.mul(seq_output, test_item_emb + user_emb).sum(dim=1)  # [B]
        return scores
